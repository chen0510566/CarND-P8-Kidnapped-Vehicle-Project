...
int main()
{
    ...
    for (int i = 0; i < num_time_steps; ++i)
    {
        ...
        else
        {
            ...
            /**@todo should add noise to the velocity and yawrate. why not directly add noise to velocity and yawrate instead of adding to position ?*/
...
#ifdef DEBUG_OUTPUT
        std::cout<<"----------------------------"<<i<<"------------------------"<<std::endl;
        std::cout<<"weight:"<<highest_weight<<"\testimation: "<<best_particle.x<<"\t"<<best_particle.y<<"\t"<<best_particle.theta<<std::endl;
        std::cout<<"ground truth:"<<gt[i].x<<"\t"<<gt[i].y<<"\t"<<gt[i].theta<<std::endl;
#endif
